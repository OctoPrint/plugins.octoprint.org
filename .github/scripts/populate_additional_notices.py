...
def get_issue(user, repo, issue):
	...
	r = requests.get(GITHUB_REST_URL + "/repos/" + user + "/" + repo + "/issues/" + issue,
	                 headers=headers)
	r.raise_for_status()
	return r.json()
	...
	if data.get("abandoned"):
		...
		if url.startswith("https://github.com"):
			...
			if len(parts) == 7 and parts[5] == "issues":
				...
				out("Abandonment URL seems to be Github ticket {}/{}#{}".format(user, repo, issue))
				...
				notices.append(dict(plugin=plugin_id,
				                    date=creation_date,
				                    ...
				                    link=url))
				out("Generated abandonment/adoption notice dated {} and pointing at {}".format(creation_date, url))
...
if __name__ == "__main__":
	...
	if notices:
		...
		notice_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "_data", "notices.yaml")
